    ...
    <body>
        ...
        <?php
            ...
            if(!empty($_POST)){
                ...
                $senha = $_POST["senha"];
                $senha = md5($senha);
                $select = "SELECT * FROM funcionario ORDER BY email";
                $resultado = mysqli_query($conexao, $select);
                $i = 0;

                while($linha=mysqli_fetch_assoc($resultado)){
                    if($email == $linha["email"]){
                       echo "<h1 class = 'text-center'>Seu email já existe no nosso banco de dados!</h1>";
                    }
                    if($ra == $linha["ra"]){
                        echo "<h1 class = 'text-center'>Seu ra já está cadastrado no nosso banco de dados!</h1>";
                    }
                    if($email != $linha["email"] && $ra != $linha["ra"]){
                        $query = "insert into funcionario(ra, nome, telefone, email, senha)
                        values('$ra', '$nome', '$telefone', '$email', '$senha')";
        ...
        <script>
            $(document).ready(function(){
                $("button[name='enviar']").click(function(){
                    ...
                    if(email=="" || email_conf=="" || senha=="" || senha_conf=="" || nome=="" || ra=="" || telefone==""){
