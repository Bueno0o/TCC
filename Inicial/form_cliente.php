    ...
    <body>
        ...
        include "conexao.php";
            if(!empty($_POST)){
                ...
                $senha = $_POST["senha"];
                $senha = md5($senha);
                $select = "SELECT cpf, email FROM cliente ORDER BY email";
                ...
                while($linha=mysqli_fetch_assoc($resultado)){
                    ...
                    if($cpf == $linha["cpf"]){
                        echo "<h1 class = 'text-center'>Seu cpf já está cadastrado no nosso banco de dados!</h1>";
                    }
                    if($email != $linha["email"] && $cpf != $linha["cpf"]){
                        $query = "insert into cliente(cpf, nome, telefone, email, senha)
